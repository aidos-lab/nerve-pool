"""
Generates configs for the mantra sweep.
Works different than the other sweep generator.

uv run src/sweep_mantra.py configs/sweep_mantra.yaml
...
def load_sweep(config_dict):
    ...
    config_list = []
    seeds = config_dict["dataset"]["seed"]
    models = config_dict["model"]

    for seed in seeds:
        for idx, model in enumerate(models):
            new_cfg = copy.deepcopy(config_dict)
            new_cfg["dataset"]["seed"] = seed
            new_cfg["model"] = model
            new_cfg["logger"]["log_dir"] = config_dict["logger"]["log_dir"][idx]
            new_cfg["logger"]["model_str"] = config_dict["logger"]["model_str"][idx]

            data_card = load_datacard(new_cfg["dataset"])
            model_config = load_model_config(new_cfg["model"] | data_card)
            new_cfg["model"] = model_config

            config_list.append(copy.deepcopy(new_cfg))
...
def main():
    ...
    config_folder = f"./configs/{config_dict['dataset']['module'].split('.')[-1]}"
    ...
    for config in configs:
        ...
        config["config"] = load_model_config(config["model"] | data_card)

        filename = f"./configs/{dataset_name}/{model_name}-{seed}.yaml"
