...
def sum_pool(
    ...
    Final node features are defined by *summing* the features of all nodes
...
def sum_pool_neighbor_x(
    data: Data,
    flow: Optional[str] = "source_to_target",
) -> Data:
    r"""Sum pools neighboring node features, where each feature in
    ...
    data.x = scatter(x[row], col, dim=0, dim_size=data.num_nodes, reduce="sum")
    return data
